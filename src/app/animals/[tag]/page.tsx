...
export default function AnimalDetailPage() {
  ...
  const getLabel = (key: string) => {
    const speciesKey =
      animal.species in keyDisplayNames ? animal.species : "default";
    return (
      keyDisplayNames[speciesKey]?.[key] || keyDisplayNames.default[key] || key
    );
  };
                ...
                {visibleFields.map((key) => (
                  ...
                  >
                    ...
                    {key === "status" ? (
                      ...
                      <p className="text-base md:text-lg font-medium text-gray-800">
                        {formatValue(key, animal[key as keyof Animal])}
                      </p>
                    )}
                  </div>
                ))}
