...
export default function LayerTable() {
  ...
  // Filter only layer with valid _id
  type LayerWithId = Layer & { _id: string };

  const layerOnly = animals.filter(
    (a): a is LayerWithId =>
      a.species === "Layer" && typeof a._id === "string"
  );

  // Search filter
  const filteredLayer = layerOnly.filter((animal) =>
  ...
  const calculateMortalityRate = (
    ...
    if (!initialFlockSize || currentFlockSize === undefined) {
      return "N/A";
    }

    return (
      (((initialFlockSize - currentFlockSize) / initialFlockSize) * 100).toFixed(
        2
      ) + "%"
    );
  };
      ...
      <div className="hidden md:block">
        <div className="border rounded-lg shadow overflow-x-auto max-h-[500px] w-full">
          <table className="w-full table-auto border-collapse text-xs md:text-sm min-w-[700px]">
            ...
            <tbody>
              {filteredLayer.length > 0 ? (
                filteredLayer.map((animal, idx) => (
                  <tr
                    key={animal._id}
            ...
            filteredLayer.map((animal) => (
              ...
              >
                ...
                <p className="text-xs text-gray-600">
                  Strains: {animal.breed}
                </p>

                <p className="text-xs text-gray-600">
                  Initial Flock Size: {animal.initialFlockSize}
                </p>

                <p className="text-xs text-gray-600">
                  Current Flock Size: {animal.currentFlockSize}
                </p>
