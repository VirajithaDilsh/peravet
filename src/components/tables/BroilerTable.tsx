...
export default function BroilerTable() {
  ...
  // Filter only broiler with valid _id
  type BroilerWithId = Broiler & { _id: string };

  const broilerOnly = animals.filter(
    (a): a is BroilerWithId =>
      a.species === "Broiler" && typeof a._id === "string"
  );
